...
fn init_bump_changelog_tests(
    include_full_release: bool,
) -> Result<(NaiveDate, String, Vec<String>)> {
    git_init()?;
    ...
    let mut shas_fixes = Vec::new();
    for i in 1..=3 {
        let sha_fix = git_commit_short(&format!("fix: fix {i}"))?;
        shas_fixes.push(sha_fix);
        Command::new(assert_cmd::cargo_bin!("cog"))
            .arg("bump")
            .arg("--auto")
            .arg("--pre")
    ...
    Ok((today, sha_init, shas_fixes))
}

#[sealed_test]
fn bump_changelog_dont_duplicate_prereleases() -> Result<()> {
    // Arrange & Act
    let (today, sha_init, sha_fixes) = init_bump_changelog_tests(false)?;
...
#[sealed_test]
fn bump_changelog_allow_empty_after_prerelease() -> Result<()> {
    // Arrange & Act
