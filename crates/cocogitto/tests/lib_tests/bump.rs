...
fn monorepo_bump_ok() -> Result<()> {
    ...
    // Act
    let result = cocogitto.create_monorepo_version(BumpOptions::default());
...
fn monorepo_bump_manual_ok() -> Result<()> {
    ...
    // Act
    let result = cocogitto.create_monorepo_version(BumpOptions {
...
fn package_bump_ok() -> Result<()> {
    ...
    // Act
    let result = cocogitto.create_package_version(PackageBumpOptions {
        package_name: "one",
        package,
...
fn should_fallback_to_0_0_0_when_there_is_no_tag() -> Result<()> {
    ...
    // Act
    let result = cocogitto.create_version(BumpOptions::default());
...
fn auto_bump_package_only_ok() -> Result<()> {
    ...
    // Act
    cocogitto.create_all_package_version_auto(BumpOptions::default())?;
...
fn error_on_no_conventional_commits_found_for_monorepo() -> Result<()> {
    ...
    // Act
    let first_result = cocogitto.create_version(BumpOptions::default());
    ...
    // Act
    let second_result = cocogitto.create_version(BumpOptions::default());
...
fn error_on_no_conventional_commits_found_for_package() -> Result<()> {
    ...
    let first_result = cocogitto.create_package_version(PackageBumpOptions {
        package_name: "jenkins",
        package: &jenkins(),
        increment: IncrementCommand::AutoPackage("jenkins".to_owned()),
        ..Default::default()
    ...
    let second_result = cocogitto.create_package_version(PackageBumpOptions {
        package_name: "jenkins",
        package: &jenkins(),
        increment: IncrementCommand::AutoPackage("jenkins".to_owned()),
        ..Default::default()
...
fn bump_order_respected_without_resolver() -> Result<()> {
    ...
    // See: https://github.com/cocogitto/cocogitto/pull/558
    ...
    let alpha = MonoRepoPackage {
        ...
        bump_order: Some(3),
        pre_bump_hooks: Some(vec![format!("echo alpha >> {log_str}")]),
        ..Default::default()
    };
    ...
    let beta = MonoRepoPackage {
        ...
        bump_order: Some(1),
        pre_bump_hooks: Some(vec![format!("echo beta >> {log_str}")]),
        ..Default::default()
    };
    ...
    let gamma = MonoRepoPackage {
        ...
        bump_order: Some(2),
        pre_bump_hooks: Some(vec![format!("echo gamma >> {log_str}")]),
        ..Default::default()
    };
